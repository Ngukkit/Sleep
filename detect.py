...
def run(weights=ROOT / './weights/best.pt',
    ...
    dlib_predictor_path = str(ROOT / 'models/shape_predictor_68_face_landmarks.dat') # 'dlib_shape_predictor' 폴더에서 'models' 폴더로 경로 변경
    dlib_analyzer = DlibAnalyzer(dlib_predictor_path) if enable_dlib else None

    mediapipe_analyzer = MediaPipeAnalyzer() if enable_mediapipe else None # MediaPipeAnalyzer 초기화
...
def parse_opt():
    ...
    parser.add_argument('--enable-mediapipe', action='store_true', help='Enable MediaPipe facial and hand landmark analysis') # MediaPipe 활성화 인자 추가
